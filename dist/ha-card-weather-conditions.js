...
        <text x="0" y="14" class="small">${e}</text>
  ...
  `}(this.hass,this._config.pollen):""}
      ...
      <div class="forecast clear">
        ${m.map(e=>{let i=new Date(r.setDate(r.getDate()+1)).toLocaleDateString(n,{weekday:"short"}),p=a?t.states[a[e][1]].state.toLowerCase():void 0,u=c?Math.round(parseFloat(t.states[c[e][1]].state)):void 0,m=l?Math.round(parseFloat(t.states[l[e][1]].state)):void 0,g=d?Math.round(parseFloat(t.states[d[e][1]].state)):void 0,_=h?Math.round(parseFloat(t.states[h[e][1]].state)):void 0;return j`
          <div class="day ${e}">
              <div class="dayname">${i}</div>
              ${p?j`
              <i class="icon" style="background: none, url('${vt(p,s,o)}') no-repeat; 
                    background-size: contain"></i>                
              `:""}
              ${Nt(u,"",m,wt(t,"temperature"))} 
              ${Nt(g,"%",_,wt(t,"precipitation")+"/h")}                       
          </div>
          `})}
      </div>
